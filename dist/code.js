...
(() => {
  ...
  function generateFakeData(placeholder) {
    ...
    switch (type) {
      ...
      case "phonenumber":
      case "phone_number":
        return `+1 (${randomInt(200, 999)}) ${randomInt(200, 999)}-${randomInt(1e3, 9999)}`;
      case "company":
      case "companyname":
      case "company_name":
      ...
      case "avatar":
      case "avatar_url":
        return `https://i.pravatar.cc/150?u=${randomInt(1, 1e3)}`;
      case "uuid":
      case "id":
  ...
  function processFakePlaceholders(text) {
  ...
  function normalizeAlignmentValue(value, defaultValue = "MIN") {
    if (!value)
      return defaultValue;
  ...
  function normalizePrimaryAxisAlignment(value, defaultValue = "MIN") {
    if (!value)
      return defaultValue;
    const upperValue = value.toUpperCase().replace(/-/g, "_");
    switch (upperValue) {
      ...
      for (const childData of data.children) {
        ...
        if (result) {
          ...
          if (childData.layoutPositioning === "ABSOLUTE" && "layoutPositioning" in result.node) {
            result.node.layoutPositioning = "ABSOLUTE";
            if (childData.constraints) {
              const node = result.node;
              if (childData.constraints.horizontal) {
                node.constraints = __spreadProps(__spreadValues({}, node.constraints), {
  ...
  function detectIconFromFrame(node) {
    var _a;
    if (node.name.toLowerCase().includes("icon:")) {
      const iconName = (_a = node.name.split(":")[1]) == null ? void 0 : _a.trim().toLowerCase();
  ...
  function serializeNode(node) {
    ...
    if (node.type === "FRAME" || node.type === "COMPONENT" || node.type === "INSTANCE") {
      ...
      if (iconName) {
        ...
        if (frame.children.length > 0) {
          ...
          if ("fills" in firstChild) {
            iconColor = getColorFromFills(firstChild.fills) || iconColor;
          }
        }
        return {
          type: "ICON",
      ...
      if (frame.layoutMode && frame.layoutMode !== "NONE") {
        ...
        const hasPadding = frame.paddingTop > 0 || frame.paddingRight > 0 || frame.paddingBottom > 0 || frame.paddingLeft > 0;
        if (hasPadding) {
          nodeData.padding = {
      ...
      if (typeof frame.cornerRadius === "number" && frame.cornerRadius > 0) {
        ...
        if (frame.topLeftRadius > 0)
          nodeData.topLeftRadius = Math.round(frame.topLeftRadius);
        if (frame.topRightRadius > 0)
          nodeData.topRightRadius = Math.round(frame.topRightRadius);
        if (frame.bottomLeftRadius > 0)
          nodeData.bottomLeftRadius = Math.round(frame.bottomLeftRadius);
        if (frame.bottomRightRadius > 0)
          nodeData.bottomRightRadius = Math.round(frame.bottomRightRadius);
      }
      const effects = serializeEffects(frame.effects);
      if (effects)
        nodeData.effects = effects;
      if (frame.opacity < 1) {
    ...
    if (node.type === "TEXT") {
      ...
      if (text.textAlignHorizontal && text.textAlignHorizontal !== "LEFT") {
        nodeData.textAlign = text.textAlignHorizontal;
      }
    ...
    if (node.type === "RECTANGLE") {
      ...
      if (hasImageFill(rect.fills)) {
        const nodeData2 = {
          ...
          src: "placeholder"
          // We can't extract the actual image URL
        };
        if (typeof rect.cornerRadius === "number" && rect.cornerRadius > 0) {
          nodeData2.cornerRadius = Math.round(rect.cornerRadius);
        }
        return nodeData2;
      }
      const nodeData = {
    ...
    if (node.type === "ELLIPSE") {
      ...
      const nodeData = {
        type: "RECTANGLE",
        // Treat as rectangle for simplicity
        name: ellipse.name,
        width: Math.round(ellipse.width),
        height: Math.round(ellipse.height),
        cornerRadius: Math.round(Math.min(ellipse.width, ellipse.height) / 2)
        // Make it circular
      };
  ...
  function countSerializedNodes(data) {
    let count = 1;
    if (data.children) {
